<template>
  <div>
    <MyCelebrationComponent :showCelebration="fireworks" />
    <div
      class="w-full max-w-[1239px] relative py-4 flex flex-col justify-end gap-y-5 pl-4 overflow-x-hidden"
    >
      <div class="gap-2 flex mt-60">
        <button @click="showSuccessToast" class="px-4 py-2 bg-green-500 text-white rounded">
          Show Success
        </button>
        <button @click="showErrorToast" class="px-4 py-2 bg-red-500 text-white rounded">
          Show Error
        </button>
        <button @click="showWarningToast" class="px-4 py-2 bg-yellow-500 text-black rounded">
          Show Warning
        </button>

        <button @click="addCoinflipKillFeed" class="px-4 py-2 bg-blue-500 text-white rounded">
          Add Coinflip Kill Feed
        </button>
        <button @click="addJackpotKillFeed" class="px-4 py-2 bg-blue-500 text-white rounded">
          Add Jackpot Kill Feed
        </button>
        <button
          @click="toggleModal('security warning')"
          class="px-4 py-2 bg-green-500 text-white rounded"
        >
          toggle warning modal
        </button>
        <button @click="toggleModal('KYC')" class="px-4 py-2 bg-green-500 text-white rounded">
          toggle KYC modal
        </button>
      </div>
      <div class="gap-2 flex justify-center mt-8">
        <button @click="dominated" class="px-4 py-2 bg-red-500 text-white rounded">
          dominating you
        </button>
        <button @click="dominating" class="px-4 py-2 bg-yellow-500 text-black rounded">
          you are dominating
        </button>

        <button @click="youRevenged" class="px-4 py-2 bg-blue-500 text-white rounded">
          you revenged
        </button>
        <button @click="revengedYou" class="px-4 py-2 bg-emerald-500 text-white rounded">
          revenged you
        </button>
      </div>

      <div class="gap-2 flex justify-center mt-8">
        <button @click="fireworks = !fireworks" class="px-4 py-2 bg-emerald-500 text-white rounded">
          start/stop fireworks
        </button>
      </div>
    </div>
  </div>
</template>
<style scoped src="../assets/css/rewards.css" />

<script>
import { openModal } from '@/modalStore'
// import Spinner from '../components/jackpot/spinner/Jackpot_Spinner.vue'
import MyCelebrationComponent from '../components/Testing_component.vue'
export default {
  components: { MyCelebrationComponent },
  data() {
    return {
      fireworks: false,
      game: {
        _id: 1,
        players: [
          {
            _id: 1,
            name: 'ILOVEBREAD',
            avatar:
              'https://letsenhance.io/static/8f5e523ee6b2479e26ecc91b9c25261e/1015f/MainAfter.jpg',
            value: 93,
            chance: 80,
            items: [
              {
                name: 'AK-47',
                image:
                  'https://community.fastly.steamstatic.com/economy/image/6TMcQ7eX6E0EZl2byXi7vaVKyDk_zQLX05x6eLCFM9neAckxGDf7qU2e2gu64OnAeQ7835Ff5GLNfDY0jhyo8DEiv5dbPK47pbcyR_m4lcnvtWQ/360fx360f',
                price: 900,
                id: 5
              }
            ],
            percentage_ticket_range: {
              lowest: 0,
              highest: 80
            },

            ticketRange: {
              min: 0,
              max: 20
            }
          },
          {
            _id: 2,
            name: 'ILOVEBREAD2',
            avatar:
              'data:image/jpeg;base64,',
            value: 0,
            chance: 20,
            items: [
              {
                name: 'AK-47',
                image:
                  'https://community.fastly.steamstatic.com/economy/image/6TMcQ7eX6E0EZl2byXi7vaVKyDk_zQLX05x6eLCFM9neAckxGDf7qU2e2gu64OnAeQ7835Ff5GLNfDY0jhyo8DEiv5dbPK47pbcyR_m4lcnvtWQ/360fx360f',
                price: 1,
                id: 1
              }
            ],
            percentage_ticket_range: {
              lowest: 80,
              highest: 100
            },
            ticketRange: {
              min: 21,
              max: 40
            }
          },
          {
            _id: 3,
            name: 'Goku',
            avatar:
              'https://image-cdn.hypb.st/https%3A%2F%2Fhypebeast.com%2Fimage%2F2018%2F02%2Fdragon-ball-super-goku-ultra-instinct-mastered-01.jpg?q=75&w=800&cbr=1&fit=max',
            value: 93,
            chance: 80,
            items: [
              {
                name: 'AK-47',
                image:
                  'https://community.fastly.steamstatic.com/economy/image/6TMcQ7eX6E0EZl2byXi7vaVKyDk_zQLX05x6eLCFM9neAckxGDf7qU2e2gu64OnAeQ7835Ff5GLNfDY0jhyo8DEiv5dbPK47pbcyR_m4lcnvtWQ/360fx360f',
                price: 900,
                id: 5
              }
            ],
            percentage_ticket_range: {
              lowest: 0,
              highest: 80
            },

            ticketRange: {
              min: 41,
              max: 60
            }
          },
          {
            _id: 4,
            name: 'Piccolo',
            avatar:
              'https://img.pastemagazine.com/wp-content/uploads/2022/08/04134921/dragon-ball-z-super-super-hero-header.jpg',
            value: 93,
            chance: 80,
            items: [
              {
                name: 'AK-47',
                image:
                  'https://community.fastly.steamstatic.com/economy/image/6TMcQ7eX6E0EZl2byXi7vaVKyDk_zQLX05x6eLCFM9neAckxGDf7qU2e2gu64OnAeQ7835Ff5GLNfDY0jhyo8DEiv5dbPK47pbcyR_m4lcnvtWQ/360fx360f',
                price: 900,
                id: 5
              }
            ],
            percentage_ticket_range: {
              lowest: 0,
              highest: 80
            },

            ticketRange: {
              min: 61,
              max: 80
            }
          },
          {
            _id: 5,
            name: 'Vegeta',
            avatar: 'https://miro.medium.com/v2/resize:fit:636/1*Z14pvsjLwMRE0KV2HhU_LA.png',
            value: 93,
            chance: 80,
            items: [
              {
                name: 'AK-47',
                image:
                  'https://community.fastly.steamstatic.com/economy/image/6TMcQ7eX6E0EZl2byXi7vaVKyDk_zQLX05x6eLCFM9neAckxGDf7qU2e2gu64OnAeQ7835Ff5GLNfDY0jhyo8DEiv5dbPK47pbcyR_m4lcnvtWQ/360fx360f',
                price: 900,
                id: 5
              }
            ],
            percentage_ticket_range: {
              lowest: 0,
              highest: 80
            },

            ticketRange: {
              min: 81,
              max: 100
            }
          }
        ]
      },
      pot_value: 400
    }
  },
  computed: {},
  mounted() {},
  methods: {
    toggleModal(name) {
      openModal(name)
    },

    demoOpen() {
      this.isOpening = true

      this.$refs.spinner.demoSpin(this.game.players[3])
    },
    showSuccessToast() {
      this.toaster.success('This is a success message!')
    },
    showErrorToast() {
      this.toaster.error('This is an error message!')
    },
    showWarningToast() {
      this.toaster.warn('This is a warning message!')
    },
    addCoinflipKillFeed() {
      this.killFeed.add('coinflip', {
        player: {
          name: 'BREAD',
          avatar: 'https://i1.sndcdn.com/artworks-BWuXyLXwYA0VEFLl-S9BFmw-t1080x1080.jpg',
          win: true,
          coin: 'heaven',
          win_streak: 1,
          dominating: 2,
          target: 1
        },

        opponent: {
          name: 'SCOOB',
          avatar:
            'https://letsenhance.io/static/8f5e523ee6b2479e26ecc91b9c25261e/1015f/MainAfter.jpg',
          win: false,
          coin: 'hell',
          win_streak: 1,
          dominating: 1,
          target: 0
        }
      })
    },
    addJackpotKillFeed() {
      this.killFeed.add('jackpot', {
        name: 'BREAD',
        message: 'WON THE $2,000 JACKPOT WITH 36% CHANCE',
        avatar:
          'https://letsenhance.io/static/8f5e523ee6b2479e26ecc91b9c25261e/1015f/MainAfter.jpg',
        details: ''
      })
    },
    dominating() {
      this.Domination_feed.add('dominating', {
        name: 'BREADISGAY',
        target_count: 4,
        domination_count: 3
      })
    },
    dominated() {
      this.Domination_feed.add('dominated', {
        name: 'BREADISGAY',
        domination_count: 3
      })
    },
    youRevenged() {
      this.Domination_feed.add('you_revenged', {
        name: 'BREADISGAY',
        target_count: 4,
        domination_count: 3
      })
    },
    revengedYou() {
      this.Domination_feed.add('revenged_you', {
        name: 'BREADISGAY',
        domination_count: 3
      })
    }
  },
  created() {
    // await this.getUserStats()
  }
}
</script>

<style scoped></style>
