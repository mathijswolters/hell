      ...
      handler(data, dataOld) {
        if (this.unboxGames.length >= 1) {
          ...
          for (const [index, game] of this.unboxGames.entries()) {
            ...
            this.unboxReels[index + 1][winnerIndex] = this.unboxGetOutcomeItem(game);
            // seed the winner early; animation will overwrite for the smooth
            // transition
            ...
            const animate = (currentTime) => {
              ...
              // update winner to whatever item is currently centered so the
              // displayed winner transitions smoothly as the reel decelerates
              this.winner = this.unboxReels[index + 1][centerIndex];
              ...
              if (progress >= 1) {
                ...
                // reveal winner after the slow settle for suspense
                setTimeout(() => {
