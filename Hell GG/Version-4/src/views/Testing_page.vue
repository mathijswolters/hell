<template>
  <div>
    <MyCelebrationComponent :showCelebration="fireworks" />
    <div
      class="w-full max-w-[1239px] relative py-4 flex flex-col justify-end gap-y-5 pl-4 overflow-x-hidden"
    >
      <div class="gap-2 flex justify-center mt-8">
      <router-link to="/chat-support">
      ...
      <div class="gap-2 flex justify-center mt-8">
        <button
          @click="toggleModal('chat unboxed')"
          class="px-4 py-2 bg-emerald-500 text-white rounded"
        >
          Chat Unboxed
        </button>
        <button
          @click="toggleModal('chat jackpot')"
          class="px-4 py-2 bg-emerald-500 text-white rounded"
        >
          Chat jackpot
        </button>
        <button
          @click="toggleModal('chat doubledown')"
          class="px-4 py-2 bg-emerald-500 text-white rounded"
        >
          Chat doubledown
        </button>
        <button
          @click="toggleModal('chat coinflip')"
          class="px-4 py-2 bg-emerald-500 text-white rounded"
        >
          Chat Coinflip
        </button>
        ...
        <button
          @click="ShowChatNotification('unbox')"
          class="px-4 py-2 bg-blue-500 text-white rounded"
        >
          Chat Unbox
        </button>
        <button
          @click="ShowChatNotification('jackpot')"
          class="px-4 py-2 bg-blue-500 text-white rounded"
        >
          Chat Jackpot
        </button>
        <button
          @click="ShowChatNotification('double-down-coinflip')"
          class="px-4 py-2 bg-blue-500 text-white rounded"
        >
          Chat Double down
        </button>
        <button
          @click="ShowChatNotification('coinflip')"
          class="px-4 py-2 bg-blue-500 text-white rounded"
        >
          Chat Coinflip
        </button>
      ...
      <div class="gap-2 flex mt-6">
        ...
        <button @click="showErrorToast" class="px-4 py-2 bg-red-500 text-white rounded">
          Show Error
        </button>
        <button
        ...
        <button
          @click="addCoinflipKillFeed"
          class="px-4 py-2 bg-blue-500 text-white rounded"
        >
          Add Coinflip Kill Feed
        </button>
        <button
          @click="addJackpotKillFeed"
          class="px-4 py-2 bg-blue-500 text-white rounded"
        >
          Add Jackpot Kill Feed
        </button>
        <button
          @click="toggleModal('security warning')"
          class="px-4 py-2 bg-green-500 text-white rounded"
        >
          toggle warning modal
        </button>
        <button
          @click="toggleModal('KYC')"
          class="px-4 py-2 bg-green-500 text-white rounded"
        >
          toggle KYC modal
        </button>
      ...
      <div class="gap-2 flex justify-center mt-8">
        <button @click="dominated" class="px-4 py-2 bg-red-500 text-white rounded">
          dominating you
        </button>
        <button @click="dominating" class="px-4 py-2 bg-yellow-500 text-black rounded">
          you are dominating
        </button>

        <button @click="youRevenged" class="px-4 py-2 bg-blue-500 text-white rounded">
          you revenged
        </button>
        <button @click="revengedYou" class="px-4 py-2 bg-emerald-500 text-white rounded">
          revenged you
        </button>
      </div>
...
<style scoped src="../assets/css/rewards.css" />

<script>
import { openModal } from "@/modalStore";
// import Spinner from '../components/jackpot/spinner/Jackpot_Spinner.vue'
import MyCelebrationComponent from "@/components/Testing_component.vue";

export default {
  components: { MyCelebrationComponent },
  data() {
    return {
      fireworks: false,
      game: {
        ...
        players: [
          ...
          },
          {
            ...
            name: "Goku",
            avatar:
              "https://image-cdn.hypb.st/https%3A%2F%2Fhypebeast.com%2Fimage%2F2018%2F02%2Fdragon-ball-super-goku-ultra-instinct-mastered-01.jpg?q=75&w=800&cbr=1&fit=max",
            value: 93,
            chance: 80,
            items: [
              {
          ...
          },
          {
            ...
            name: "Piccolo",
            avatar:
              "https://img.pastemagazine.com/wp-content/uploads/2022/08/04134921/dragon-ball-z-super-super-hero-header.jpg",
            value: 93,
            chance: 80,
            items: [
              {
          ...
          },
          {
            ...
            name: "Vegeta",
            avatar:
              "https://miro.medium.com/v2/resize:fit:636/1*Z14pvsjLwMRE0KV2HhU_LA.png",
            value: 93,
            chance: 80,
            items: [
              {
  ...
  methods: {
    ...
    demoOpen() {
      ...
      this.$refs.spinner.demoSpin(this.game.players[3]);
    },
    showSuccessToast() {
      this.toaster.success("This is a success message!");
    },
    showErrorToast() {
      this.toaster.error("This is an error message!");
    },
    showWarningToast() {
      this.toaster.warn("This is a warning message!");
    },
    ShowChatNotification(type) {
      if (type === "unbox") {
        this.chatNotification.add("unbox", {
          name: "COOLER",
          avatar: "/img/icons/clever-boy.png",
          case_name: "LVL 95 Case",
          case_image: "/src/assets/img/case_image.png",
          item_image: "/img/icons/shoes.png",
          item_icon: "/img/ranks/2.png",
          price: "500.00",
          chance: "22.52",
        });
      }
      ...
      if (type === "jackpot") {
        this.chatNotification.add("jackpot", {
          name: "COOLER",
          avatar: "/img/icons/clever-boy.png",
          item_icon: "/img/ranks/2.png",
          amount: "2000",
      ...
      if (type === "double-down-coinflip") {
        this.chatNotification.add("double-down-coinflip", {
          name: "COOLER",
          avatar: "/img/icons/clever-boy.png",
          item_icon: "/img/ranks/2.png",
          amount: "2000",
      ...
      if (type === "coinflip") {
        this.chatNotification.add("coinflip", {
          name: "COOLER",
          avatar: "/img/icons/clever-boy.png",
          item_icon: "/img/ranks/2.png",
          amount: "2000",
    ...
    addJackpotKillFeed() {
      this.killFeed.add("jackpot", {
        name: "BREAD",
        message: "WON THE $2,000 JACKPOT WITH 36% CHANCE",
        avatar:
          "https://letsenhance.io/static/8f5e523ee6b2479e26ecc91b9c25261e/1015f/MainAfter.jpg",
        details: "",
      });
    ...
    dominating() {
      this.Domination_feed.add("dominating", {
        name: "BREADISGAY",
        target_count: 4,
        domination_count: 3,
      });
    ...
    dominated() {
      this.Domination_feed.add("dominated", {
        name: "BREADISGAY",
        domination_count: 3,
      });
    },
    youRevenged() {
      this.Domination_feed.add("you_revenged", {
        name: "BREADISGAY",
        target_count: 4,
        domination_count: 3,
      });
    ...
    revengedYou() {
      this.Domination_feed.add("revenged_you", {
        name: "BREADISGAY",
        domination_count: 3,
      });
    },
