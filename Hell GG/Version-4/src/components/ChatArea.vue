    ...
    >
      <div class="flex w-full flex-col">
        <div class="w-full flex gap-[0.5rem]" v-for="message in messages" :key="message">
          <div
            v-if="message.type == 'hof announcement'"
            class="w-full bg-[#590101] flex flex-col"
          >
            <div
            ...
            <div class="h-[51px] flex items-center justify-center gap-1">
              ...
              <img
                ...
                class="max-h-[20px]"
              />
              <span class="font-Rubik text-white font-bold text-sm">{{
                message.user.name
              }}</span>
          ...
          <div v-else class="w-full flex gap-[0.5rem] p-2">
            ...
            <span
              class="w-full break-words text-white text-[14px] font-medium font-Rubik pt-1"
            >
              <span
                ...
                :class="{
                  ...
                  'text-white':
                    message.user.role != 'mod' && message.user.role != 'admin',
                }"
              >
                <span
                ...
                <img
                  ...
                  class="max-h-4 mr-1"
                  :class="{
                    '-ml-1': message.user.role !== 'admin' && message.user.role !== 'mod',
                  }"
                />
                {{ message.user.name }}
      ...
      <div class="flex items-center justify-between gap-[1rem] relative">
        ...
        >
          ...
          <ul class="list-disc pl-4 text-white font-medium font-Rubik text-sm">
            <li class="text-white font-medium font-Rubik text-sm">
              No Bigotry/Racism/Sexism.
            </li>
            <li class="text-white font-medium font-Rubik text-sm">No Begging.</li>
            <li class="text-white font-medium font-Rubik text-sm">
              No referral codes/links.
            </li>
          </ul>
        </div>
        <button
  ...
  methods: {
      ...
      if (messageContent !== "") {
        ...
        this.messages.push({
          ...
          user: {
            ...
            target: 3,
            avatar:
              "https://static0.gamerantimages.com/wordpress/wp-content/uploads/2022/07/Super-Gogeta.jpg",
          },
        });
    ...
    formatMessage(message) {
      ...
      return message.replace(/:([a-zA-Z0-9_]+):/g, (match, name) => {
        const emoji = this.emojis.find(
          (e) => e.name.toLowerCase() === name.toLowerCase()
        );
        if (emoji) {
          let emojiTag = `<img src="/img/emojis/${emoji.file}" alt="${emoji.name}" class="emoji" style="max-width: 1.6em !important;vertical-align: middle !important;display: inline-block !important;margin: 0 0em !important;height: auto !important;">`;

          if (/^:[a-zA-Z0-9_]+:$/.test(message.trim())) {
            emojiTag = `<img src="/img/emojis/${emoji.file}" alt="${emoji.name}" class="big-emoji" style="max-width: 2.5em !important;vertical-align: middle !important;display: inline-block !important;margin: 0 0.2em !important;height: auto !important;">`;
          }
...
.discord-chat-wrap:hover .discord-btn {
  display: flex;
}
